  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        if (response?.data) {
          const { summary, by_machine, by_error_type, time_evolution } = response.data;
          ...
          setSummaryData({
            ...
            avg_downtime: summary.total_incidents ? 
              Math.round(summary.total_downtime / summary.total_incidents) : 0,
          ...
          setTopMachines([
            ...
            { name: "Komax Alpha 488 S 7M (ALPHA 146)", total_downtime: 85, incident_count: 3 },
            { name: "Komax Alpha 355 (ALPHA 176)", total_downtime: 45, incident_count: 2 }
          ]);
          ...
          setTopErrorTypes([
            ...
            { error_type: "1 Mechanical (04 Blockage)", total_downtime: 100, incident_count: 5 },
            { error_type: "7 Inspection (Preventive)", total_downtime: 80, incident_count: 3 }
          ]);
        }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-sm">
          ...
          <p className="text-sm text-gray-600">Temps d'arrêt: {formatDuration(data.total_downtime)}</p>
          <p className="text-sm text-gray-600">Incidents: {data.incident_count}</p>
          <p className="text-sm text-gray-600">Moyenne: {formatDuration(Math.round(data.total_downtime / data.incident_count))}</p>
        </div>
      );
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-lg font-semibold mb-4">Principales machines avec temps d'arrêt</h2>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {isLoading ? (
              ...
              {recentReports.map((report) => (
                <div key={report.id} className="py-3">
                  ...
                  <div className="mt-1 text-sm text-gray-600">
                    Temps d'arrêt total: {formatDuration(report.total_downtime_minutes)}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="py-8 text-center text-gray-500">
              ...
              <Link to="/upload" className="mt-2 inline-block text-blue-600 hover:text-blue-800 font-medium">
          ...
          <div className="mt-4 text-right">
            <Link to="/upload" className="text-blue-600 hover:text-blue-800 text-sm font-medium">
